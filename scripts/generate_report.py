...
add_para(
    ...
    "在自建的 110 条评测集上进行了四组消融与查询增强实验，结果表明完整混合检索管线（BM25 + 向量 + RRF + Reranker）"
    "在 top-1 命中率（74.55%）、MRR（0.7889）与 nDCG@5（0.8026）三项指标上均达最优，"
    "LLM 查询改写进一步将复杂查询 top-1 提升约 9 个百分点，验证了混合检索与查询增强策略的有效性；"
...
add_bullet("建立检索评测体系：构建 110 条评测集与 402 分片知识库（含真实公开语料），开展四组消融与查询增强（改写 / HyDE）消融实验，并接入 RAGAS 评测生成质量与检索性能基准。")
...
add_heading("7.3 检索性能评测（四组消融 + 查询增强）", 2)
add_para(
    "为量化评估检索质量，系统构建了 110 条评测集，覆盖基线直配、同义改写、跨语言、多主题干扰、"
    "反向否定五类难度（各 22 条），每条记录期望关键词（库内 chunk 的真实子串）。"
    "知识库扩充至 402 个分片、22 份文档，覆盖智能客服、工业物联网、信息安全、人力资源、数据仓库、"
    "向量数据库、推荐系统、财务、IT 运维、数据合规等合成多主题文档，并混合 pgvector、LangChain、"
    "FastAPI 等真实公开文档片段，制造检索干扰。"
...
add_para(
    "为进一步提升复杂查询的检索效果，在完整管线的查询侧加入两类增强：LLM 查询改写（将原始问题改写"
    "为多个语义变体分别检索后 RRF 合并）与 HyDE（生成假设回答文档、以其向量替代原始查询向量做检索）。"
    "三者在完整管线上的对比结果如下表所示。"
)
add_table(
    ["指标", "baseline", "+查询改写", "+HyDE"],
    [
        ["top-1 命中率（全量）", "74.55%", "80.00%", "74.55%"],
        ["MRR（全量）", "0.7889", "0.8321", "0.7870"],
        ["nDCG@5（全量）", "0.8026", "0.8446", "0.8031"],
        ["top-1（复杂查询 66 条）", "62.12%", "71.21%", "63.64%"],
        ["nDCG@5（复杂查询 66 条）", "0.7022", "0.7710", "0.7106"],
    ],
    col_widths=[6.4, 3.0, 3.0, 3.0],
)
add_para(
    "结果表明：查询改写显著提升复杂查询（多主题干扰、跨语言、反向否定）的检索效果，"
    "复杂查询子集上 top-1 由 62.12% 提升至 71.21%（+9.09 个百分点）、nDCG@5 由 0.7022 提升至 0.7710，"
    "说明改写产生的多路召回能更充分地覆盖问题语义；HyDE 在本语料上与 baseline 基本持平，"
    "作为对照如实呈现。"
)

add_heading("7.4 检索性能基准", 2)
add_para(
    "为评估检索的实时性与吞吐能力，对 30 条采样查询做端到端延迟与分阶段耗时测量（CPU 推理、top-5），"
    "结果如下表所示。"
)
add_table(
    ["配置", "P50", "P95", "均值"],
    [
        ["Reranker OFF", "51.60ms", "54.26ms", "54.42ms"],
        ["Reranker ON", "1362.92ms", "1680.06ms", "1460.17ms"],
    ],
    col_widths=[4.0, 3.8, 3.8, 4.0],
)
add_para(
    "分阶段耗时（Reranker OFF）显示：向量检索（pgvector）占 93.3%（约 48ms），BM25 稀疏检索仅 0.29ms，"
    "验证了倒排索引剪枝的高效性。Reranker 精排是端到端延迟的绝对瓶颈，约占 90%（单次约 1.31 秒），"
    "这是「以延迟换首位精度」的又一体现；并发吞吐（1/4/8）为 0.52 / 0.76 / 0.80 queries/s，"
    "受 CPU 推理并行度限制。"
)
add_para(
    "此外，针对 BM25 索引维护做了全量重建与增量更新的耗时对比：385 分片全量重建约 52.93ms，"
    "而增量新增 49 个分片仅 7.54ms、增量删除 0.37ms，且增量耗时与语料总量无关，"
    "验证了增量更新对大规模知识库的必要性。"
)

add_heading("7.5 中文分词对比", 2)
add_para(
...
add_heading("7.6 生成质量评测（RAGAS）", 2)
add_para(
    "为评估端到端生成质量，接入 RAGAS 框架，采用 LLM-as-judge 对 44 条真实问答样本评测四项指标："
...
add_para(
    "faithfulness 为 0.69，表明生成的回答整体忠于检索上下文、无编造；answer_relevancy 为 0.62，"
    "随查询难度上升（同义改写、跨语言、多主题干扰等难例）而有所下降，符合预期；"
    "context_precision（0.79）与 context_recall（0.84）表明检索上下文具备良好的覆盖与排序质量，"
    "且在不同难度样本上呈现合理区分度。"
)

add_heading("7.7 多轮对话与指代消解测试", 2)
add_para(
...
add_heading("7.9 测试结论", 2)
add_para(
    "综合功能测试与评测结果，系统各功能模块运行正常，端到端流程通畅；"
    "四组消融实验验证了混合检索与 Reranker 精排的贡献，完整管线在 top-1 命中率（74.55%）、"
    "MRR（0.7889）与 nDCG@5（0.8026）上均达最优，查询改写进一步将复杂查询 top-1 提升至 71.21%；"
    "jieba 分词在保持命中率基本持平的同时将 BM25 索引缩减约 34%，BM25 索引支持增量更新；"
    "RAGAS 评测表明生成回答的忠实度与相关性良好；"
    "检索性能基准定位了 Reranker 精排的延迟瓶颈，为后续 GPU 加速提供依据。"
...
add_para(
    ...
    "验证了 Reranker 精排对检索质量的显著提升；引入 LLM 查询改写与 HyDE 检索增强，"
    "实现了 BM25 索引增量更新与检索性能基准。"
)

add_heading("8.2 系统不足与局限", 2)
add_para(
    "系统仍存在以下不足：（1）BM25 倒排索引虽已支持增量更新，但仍为进程内存结构，"
    "重启后需重建，未持久化到 PostgreSQL tsvector 等外部索引；（2）Embedding 与 Reranker 在 CPU 上推理，"
    "Reranker 精排带来显著延迟（单次约 1.4 秒），大批量入库与高并发下吞吐受限；"
    "（3）评测语料以程序化合成的企业文档为主，真实公开文档占比较小，与真实企业知识库分布仍有差异；"
    "（4）RAGAS 评测样本规模仍有限（44 条），端到端生成质量的评估仍有待更大规模真实数据的验证。"
)

add_heading("8.3 未来展望", 2)
add_para(
    "后续工作可从以下方向展开：将 BM25 倒排索引持久化到 PostgreSQL tsvector 或 Elasticsearch，"
    "避免重启重建并支撑更大规模数据；支持 GPU 推理与模型服务化部署，缓解 Reranker 精排带来的延迟瓶颈；"
    "扩充更大规模的真实企业文档评测语料与 RAGAS 样本，使评测更贴近生产环境；"
    "探索更多进阶检索策略以进一步提升复杂问题的回答质量。"
)
